...
export function ContactScreen({ onNavigate }: ContactScreenProps) {
  ...
  const getDirectQueryMessageText = () => {
    return `📩 NEW DIRECT CLINIC QUERY
...
📍 Sent from Pearl Clinic Website
Helpline: +91 9981342401
Email: pearlclinic.jbp@gmail.com`;
  };

  const doctorWhatsappUrl = `https://wa.me/919981342401?text=${encodeURIComponent(
    getDirectQueryMessageText()
  )}`;
  const doctorSmsUrl = `sms:+919981342401?body=${encodeURIComponent(
    getDirectQueryMessageText()
  )}`;
  const clinicEmailUrl = `mailto:pearlclinic.jbp@gmail.com?subject=${encodeURIComponent(
    `Direct Query from ${queryData.name}`
  )}&body=${encodeURIComponent(getDirectQueryMessageText())}`;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!queryData.name || !queryData.phone || !queryData.message) return;
    ...
    try {
      ...
      console.log("Backend offline, dispatched via client protocols", err);
    }
    ...
    <div className="space-y-10 pb-12">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        ...
        <p className="text-slate-300 text-sm">
          Reach Dr. Diksha Asati & Dr. Rahul Asati at Pearl Clinic, Vijay Nagar, Jabalpur for OPD appointments or urgent care.
        </p>
      </div>
      ...
      {/* Emergency Banner */}
      <div className="p-6 rounded-3xl bg-gradient-to-r from-rose-950/80 via-slate-900 to-rose-950/80 border border-rose-500/30 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
        <div className="flex items-center gap-4">
          <div className="p-4 rounded-2xl bg-rose-500/20 text-rose-400 animate-pulse">
            <Ambulance className="w-8 h-8" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white">24/7 Medical Emergency Hotline</h2>
            <p className="text-xs text-rose-300">
              Direct Helpline: <span className="font-bold text-white">+91 9981342401</span> | For fever, breathing distress, asthma or urgent care.
            </p>
          </div>
        </div>
        <a
          href="tel:+919981342401"
          className="px-6 py-3.5 rounded-2xl bg-rose-500 hover:bg-rose-400 text-white font-bold text-sm shadow-xl shadow-rose-500/30 flex items-center gap-2 transition-transform transform hover:scale-105 shrink-0"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="space-y-4 text-sm text-slate-300">
              <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800/80 space-y-1">
                ...
                <p className="font-medium text-white">
                  Pearl Clinic, Scheme No 54, Vijay Nagar, Jabalpur, Madhya Pradesh 482002
                </p>
                <p className="text-xs text-slate-400">Direct Navigation & Photos: Vijay Nagar, Jabalpur</p>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800/80 space-y-1">
                <span className="text-xs font-semibold text-emerald-400 uppercase tracking-wider">OPD Consultation Days</span>
                <p className="font-bold text-white">Monday to Saturday</p>
                <p className="text-xs text-slate-400">Morning & Evening OPD Sessions</p>
              </div>
            </div>

            {/* Interactive Map Visual Box */}
            <div className="relative w-full h-56 rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center p-6 text-center">
        ...
        <div className="lg:col-span-5">
          <div className="p-6 rounded-3xl glass-card border border-slate-800 space-y-6">
            <h2 className="text-xl font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
              <MessageSquare className="w-5 h-5 text-emerald-400" /> Direct Query & Message
            </h2>

            {submitted ? (
              <div className="p-6 text-center space-y-4 bg-slate-950 rounded-2xl border border-emerald-500/40 shadow-xl">
                <div className="w-14 h-14 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mx-auto border border-emerald-500/30">
                  <CheckCircle2 className="w-7 h-7" />
                </div>
                <div>
                  <h3 className="font-black text-emerald-400 text-lg">Message Sent Successfully!</h3>
                  <p className="text-xs text-slate-300 mt-1">
                    Your direct message from <strong className="text-white">{queryData.name}</strong> ({queryData.phone}) has been processed and automatically dispatched via WhatsApp, SMS, and Email.
                  </p>
                </div>

                {/* Automated Backend Dispatch Summary */}
                <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 text-left space-y-2 text-xs">
                  <div className="font-semibold text-slate-300 border-b border-slate-800 pb-1">Automated Backend Dispatch Log:</div>
                  <div className="flex items-center gap-2 text-emerald-400 font-medium">
                    <CheckCircle2 className="w-3.5 h-3.5" /> WhatsApp to Patient (+91 {queryData.phone})
                  </div>
                  <div className="flex items-center gap-2 text-emerald-400 font-medium">
                    <CheckCircle2 className="w-3.5 h-3.5" /> WhatsApp to Doctor Desk (+91 9981342401)
                  </div>
                  <div className="flex items-center gap-2 text-emerald-400 font-medium">
                    <CheckCircle2 className="w-3.5 h-3.5" /> SMS to Patient (+91 {queryData.phone})
                  </div>
                  <div className="flex items-center gap-2 text-emerald-400 font-medium">
                    <CheckCircle2 className="w-3.5 h-3.5" /> SMS to Doctor Desk (+91 9981342401)
                  </div>
                  <div className="flex items-center gap-2 text-emerald-400 font-medium">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Email to pearlclinic.jbp@gmail.com
                  </div>
                </div>

                {/* Instant Actions for WhatsApp, SMS (9981342401), Email */}
                <div className="space-y-2 text-left pt-1">
                  <a
                    href={doctorWhatsappUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-white border border-emerald-500/30 text-xs font-bold flex items-center justify-between transition-colors"
                  >
                    <span className="flex items-center gap-2">
                      <Smartphone className="w-4 h-4" /> Open WhatsApp Chat (9981342401)
                    </span>
                    <span>↗</span>
                  </a>
                  ...
                  <a
                    href={doctorSmsUrl}
                    className="p-3 rounded-xl bg-sky-500/10 hover:bg-sky-500 text-sky-300 hover:text-white border border-sky-500/30 text-xs font-bold flex items-center justify-between transition-colors"
                  >
                    <span className="flex items-center gap-2">
                      <Phone className="w-4 h-4" /> Send Text / SMS (9981342401)
                    </span>
                    <span>↗</span>
                  </a>
                  ...
                  <a
                    href={clinicEmailUrl}
                    className="p-3 rounded-xl bg-rose-500/10 hover:bg-rose-500 text-rose-300 hover:text-white border border-rose-500/30 text-xs font-bold flex items-center justify-between transition-colors"
                  >
                    <span className="flex items-center gap-2">
                      <Mail className="w-4 h-4" /> Email pearlclinic.jbp@gmail.com
                    </span>
                    <span>↗</span>
                  </a>
                </div>
                ...
                <div className="pt-2 border-t border-slate-900">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-xs text-sky-400 font-semibold cursor-pointer"
                  >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs focus:border-emerald-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    required
                    placeholder="Ask about clinic timing, doctor availability, or vaccination stock..."
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-xs shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 cursor-pointer"
                >
      ...
      <section className="p-8 rounded-3xl glass-card border border-slate-800 space-y-6">
          ...
          {clinicPhotos.map((photo, idx) => (
            ...
            >
              <div className="relative h-64 w-full overflow-hidden">
                ...
                <span className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-slate-900/80 border border-slate-700 text-[10px] font-bold text-sky-300 uppercase tracking-wider">
                  {photo.category}
                </span>
              </div>
              <div className="p-4 space-y-1">
                <h3 className="font-bold text-xs text-white group-hover:text-sky-300 transition-colors">
                  {photo.title}
                </h3>
                <p className="text-[11px] text-slate-400">Click to enlarge authentic photo</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {selectedPhoto && (
          ...
          >
            <div className="relative max-w-4xl w-full max-h-[90vh] rounded-3xl overflow-hidden border border-slate-700 bg-slate-900 shadow-2xl p-2 flex flex-col items-center">
              <button
                onClick={() => setSelectedPhoto(null)}
